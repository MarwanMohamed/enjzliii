...
<tbody>
    ...
    <tr>
        ...
        <td>{{$bid->reports->count()}}</td>
        <td>{{bidStatus($bid,$bid->project,$setting)}}</td>
        <td>{{$bid->reports[$key]->isView == 0  ? 'غير مقروء' : 'مقروء'}}</td>
        <td> 
            @if(checkPerm('show'))<a href="/admin/reports/show/5/{{$bid->id}}" title="عرض" class=" btn btn-success btn-xs"><span
