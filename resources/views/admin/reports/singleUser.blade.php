    ...
    <div class="row">
        <div class="col-sm-3">
          <div class="panel panel-default">
            ...
            <img style="width: 140px;" style="width: 140px;"  src="{{$user->avatar()}}" class="thumbnail img-responsive" alt=""/>
            <div class="mb30"></div>
            <h5 class="subtitle">صاحب العمل</h5>
            <address style="line-height: 24px;font-size: 14px;" class='address_sub'>
                <a href="/admin/users/single/{{$user->id}}"> <span>اسم المستخدم :</span>{{$user->username?$user->username:'غير مدخل'}}</a><br>
                <a href="/admin/users/single/{{$user->id}}"> <span>البريد الإلكتروني :</span> {{$user->email}} </a><br>
                <a href="/admin/users/single/{{$user->id}}"> <span>الملف الشخصي :</span> {{$user->isFinishProfile?'جاهز':'غير جاهز'}}</a><br>
            </address>
        </div>
            </div>
          </div>
            ...
            <div class="profile-header">
              <div class="table-responsive">
                <table class="table" style="text-align: right">
                    ...
                    <tr>
                        <td>عدد المشاريع</td>
                        <td>{{$user->myProjects->count()}}</td>
                    </tr>
                    @if(checkPermFull('/admin/users/unblock')||checkPerm('blockUser'))
                    <tr>
                        <td>
                            @if(!$user->status==2)
                            <a class="btn btn-danger" href="/admin/reports/blockUser/{{$user->id}}">حظر</a>
                            @endif
                        </td>
                    </tr>
                    @endif
    ...
    <div id="cancelModel" class="modal fade" role="dialog">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form action="/admin/users/cancel" onsubmit="cancelSubmit()" id="cancelForm" method="post">
                    {{csrf_field()}}
    ...
    <script>
        $(function () {
            $('.cancelProject').click(function (e) {
                e.preventDefault();
                $('#cancelModel').find('#user_id').val($(this).data('id'));
