...
<tbody>
    ...
    <tr>
        ...
        <td><a href='/admin/users/single/{{$user->id}}'>{{$user->fullname()}}</a></td>
        <td>{{$user->email}}</td>
        <td>{{$user->myProjects->count()}}</td>
        <td>{{$user->reports->count()}}</td> 
        <td>{{$user->reports[$key]->isView == 0  ? 'غير مقروء' : 'مقروء'}}</td>
        <td> 
            @if(checkPerm('show'))<a href="/admin/reports/show/3/{{$user->id}}" title="عرض" class=" btn btn-success btn-xs"><span
