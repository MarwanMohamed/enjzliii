    ...
    <div class="signinpanel">
        ...
        <div class="row">
            <div class="col-md-6 col-md-offset-3">
                ...
                {!! Form::open(['url' => route('handle_resetPass'), 'class' => '']) !!}
                    <h4 class="nomargin">إعادة تعين كلمة المرور</h4>
                    <?php if(session()->has('msg')){ ?>
                        <h5 class="alert-danger alert"><?=session()->get('msg') ?></h5>
                    <?php }?>
                    <input type="password"  name="password" class="form-control uname" placeholder="كلمة المرور" />
                    <input type="hidden" value="{{$token}}"  name="token" class="form-control uname" placeholder="كلمة المرور" />
                    <input type="password"  name="password_confirmation" class="form-control uname" placeholder="تأكيد كلمة المرور" />
                    {{ csrf_field() }}
