...
abstract class Server
{
    ...
    protected function fetchUserDetails(TokenCredentials $tokenCredentials, $force = true)
    {
        if (!$this->cachedUserDetailsResponse || $force) {
            ...
            try {
                ...
                throw new \Exception(
                    "Received errors [$body] with status code [$statusCode] when retrieving token credentials."
                );
            }
    ...
    protected function createTokenCredentials($body)
    {
        ...
        if (isset($data['errors'])) {
            throw new CredentialsException("Error [{$data['errors']}] in retrieving token credentials.");
        }
