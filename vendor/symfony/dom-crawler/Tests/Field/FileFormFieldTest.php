...
class FileFormFieldTest extends FormFieldTestCase
{
    public function testInitialize()
    {
        ...
        $this->assertEquals(array('name' => '', 'type' => '', 'tmp_name' => '', 'errors' => UPLOAD_ERR_NO_FILE, 'size' => 0), $field->getValue(), '->initialize() sets the value of the field to no file uploaded');
    ...
    public function testSetValue($method)
    {
        ...
        $field->$method(null);
        $this->assertEquals(array('name' => '', 'type' => '', 'tmp_name' => '', 'errors' => UPLOAD_ERR_NO_FILE, 'size' => 0), $field->getValue(), "->$method() clears the uploaded file if the value is null");
        ...
        $this->assertEquals(0, $value['errors'], "->$method() sets the errors of the file field");
    ...
    public function testSetErrorCode()
    {
        ...
        $value = $field->getValue();
        $this->assertEquals(UPLOAD_ERR_FORM_SIZE, $value['errors'], '->setErrorCode() sets the file input field errors code');

        try {
            $field->setErrorCode('foobar');
            $this->fail('->setErrorCode() throws a \InvalidArgumentException if the errors code is not valid');
        } catch (\InvalidArgumentException $e) {
            $this->assertTrue(true, '->setErrorCode() throws a \InvalidArgumentException if the errors code is not valid');
        }
    }
